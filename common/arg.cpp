...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--lora-seq-routing"},
        "route each slot to one adapter of the --lora pool, so one decode can carry a mix of adapters (default: disabled)",
        [](common_params & params) {
            params.lora_seq_routing = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_LORA_SEQ_ROUTING"));
    add_opt(common_arg(
